	...
	<form action="#" id="submit_form" class="form-horizontal form-validate">
		<div class="form-body">
            ...
            <div class="form-group">
				<label class="control-label col-md-2" for="title">推送名称<span class="required">*</span></label>
				<div class="col-md-3">
					<input type="text" name="title" id="title" placeholder="请输入推送名称" data-required="1" class="form-control" 
                        value="<% if (ViewData["title"] != null) { %><%= ViewData["title"] %><% } %>"/>
				</div>
			</div>
            <div class="form-group">
				<label class="control-label col-md-2" for="linkurl">链接URL<span class="required">*</span></label>
				<div class="col-md-3">
					<input type="text" name="linkurl" id="linkurl" placeholder="请输入链接URL" data-required="1" class="form-control" 
                        value="<% if (ViewData["linkurl"] != null) { %><%= ViewData["linkurl"] %><% } %>"/>
				</div>
			</div>            
	...
	<script>
	    ...
	    function redirectToListPage(status) {
	        if (status.indexOf("error") != -1) {
	            ...
	            window.location = rootUri + "Banner/Banner";
	        }
	    }
	    ...
	    function submitform() {
	        $.ajax({
	            //async: false,
	            type: "POST",
	            url: rootUri + "Banner/SubmitBanner",
	            dataType: "json",
